...
_CACHE: dict = {}  # in-memory cache keyed by scheme_code
...
class MFAnalyzer:
    ...
    def _fetch_nav_history(self, scheme_code: int):
        ...
        try:
            ...
            if data.get("status") == "SUCCESS":
                _CACHE[scheme_code] = data
                return data
        except Exception:
            pass
        return None

    def _nav_on_date(self, nav_data: list, target_date: datetime):
    ...
    def _score(self, returns: dict, category: str, expense_ratio: float) -> float:
        ...
        # 3-year return (most important for long-term)
        ...
        if expense_ratio and expense_ratio > 0:
            total += 15
            if expense_ratio <= 0.3:    score += 15   # index/direct ultra-low
    ...
    def _recommend(self, score: float, category: str,
                   monthly_sip: float, returns: dict) -> tuple:
        r3 = returns.get("return_3yr")
        r5 = returns.get("return_5yr")
        ...
        # Extra warning for poor 5-year track record
    ...
    def analyze(self, fund_name: str, scheme_code: int, monthly_sip: float,
                ...
                current_nav_csv: float = None,
                current_value_csv: float = None,
                pnl_csv: float = None,
                category_hint: str = None) -> dict:

        invested = round(total_units * avg_purchase_nav, 2)

        # Pre-fill from CSV data (Zerodha Coin export already has current values)
        cur_val  = current_value_csv if current_value_csv else None
        ...
        result = {
            ...
            "score":            None,
            "action":           "HOLD",
            "reason":           "Historical NAV data unavailable — advice based on fund category only.",
            "sip_suggestion":   "",
        }

        # Try to enrich with historical NAV data from mfapi.in
        raw = self._fetch_nav_history(scheme_code) if scheme_code else None
        ...
        if raw:
            ...
            # Use API NAV if CSV NAV not provided
            if current_nav and not current_nav_csv:
                result["current_nav"]   = round(current_nav, 4)
                result["current_value"] = round(current_nav * total_units, 2)
                result["gain_loss"]     = round(result["current_value"] - invested, 2)
                if invested > 0:
                    result["gain_loss_pct"] = round((result["current_value"] / invested - 1) * 100, 2)
        ...
        # If no historical data, give category-based advice
        if not returns:
            action, reason, sip_note = self._category_advice(category, monthly_sip, gain_pct)

        result["action"]         = action
        result["reason"]         = reason
        result["sip_suggestion"] = sip_note

        return result

    def _category_advice(self, category: str, monthly_sip: float, gain_pct: float = None) -> tuple:
